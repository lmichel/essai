...
public class CadmsdExplorer {
	 ...
	 */
	public static final String IVOA_REAL_QUANTITY_VALUE ="ivoa:RealQuantity.value";
	/**
	 ...
	 * {@link CabmsdReport} is an internal class
	 */
	private CabmsdReport cabmsdReport = new CabmsdReport();

	/**
	 * Constructor: trigger data parsing
	 * @throws Exception
	 */
	CadmsdExplorer() throws Exception{
		this.initParser();
	}
	...
	private void initParser() throws Exception {
		URL url = CadmsdExplorer.class.getResource(VOTABLE_RESOURCE);
	...
	public void exploreDataSet() throws Exception{	
		...
		this.cabmsdReport.sourceId = dataSet.getStringValue();
		dataSet = this.liteMappingParser.getFirstNodeWithRole("meas:EquatorialPosition.ra");
		this.cabmsdReport.ra = dataSet.getSubelementsByRole(IVOA_REAL_QUANTITY_VALUE).get(0).getStringValue();
		dataSet = this.liteMappingParser.getFirstNodeWithRole("meas:EquatorialPosition.dec");
		this.cabmsdReport.dec = dataSet.getSubelementsByRole(IVOA_REAL_QUANTITY_VALUE).get(0).getStringValue();
		
		this.cabmsdReport.docURL = this.liteMappingParser.getFirstNodeWithRole("cab-msd:WebUrl.url").getStringValue();
	}
	/**
	...
	public void exploreFilters() throws Exception{
		...
		this.cabmsdReport.filterUrl = dataSet.getStringValue();
		dataSet = this.liteMappingParser.getFirstNodeWithRole("cab-msd:Filter.name");
		this.cabmsdReport.filterName = dataSet.getStringValue();
	}	
	/**
	...
	public void exploreData() throws Exception{		
		MappingElement pointArray = this.liteMappingParser.getFirstNodeWithRole("nd_point:points");
		this.cabmsdReport.nbPoints = pointArray.getLength();
		this.cabmsdReport.columnMapping = pointArray.getColumnRoles();
        for( int i=0 ; i<this.cabmsdReport.nbPoints ; i++ ){
        	MappingElement row = pointArray.getContentElement(i);
        	this.cabmsdReport.points.add(
        			new Point(
        					row.getSubelementsByRole(IVOA_REAL_QUANTITY_VALUE).get(0).getStringValue(),
	...
	public void printReport(){
		...
		System.out.println("        id:" + this.cabmsdReport.sourceId);
		System.out.println("  position:" + this.cabmsdReport.ra + " " + this.cabmsdReport.dec);
		System.out.println("       doc:" + this.cabmsdReport.docURL);
		System.out.println("== Filter ==");
		System.out.println("      name:" + this.cabmsdReport.filterName);
		System.out.println("       url:" + this.cabmsdReport.filterUrl);
		System.out.println("== Time Series ==");
		System.out.println("  nb points:" + this.cabmsdReport.nbPoints);
		System.out.print("col mapping: ");
		for( Entry<Integer, String> entry : this.cabmsdReport.columnMapping.entrySet()){
			System.out.print(entry.getKey() + "=>" + entry.getValue() + " ");
		}
		System.out.println("");
		for( Point point : this.cabmsdReport.points){
	...
	public static void main(String[] args) throws Exception {
		CadmsdExplorer timeSeriesExample = new CadmsdExplorer();
	...
	/*
	 * GNUplot script run on http://gnuplot.respawned.com/
	 * 
	 * 
...
set label 2 "{/:Bold Position} = 298.00252119999999 29.871491800000001"
set label 2 at graph 0.04, 0.80 tc lt 3 font "Times-Roman,12"
set label 3 '{/:Bold Filter} = G-Band ({/:Italic http://svo.url.filter/zdf/g})'
set label 3 at graph 0.04, 0.75 tc lt 3 font "Times-Roman,12"
set label 4 '{/:Bold Doc} = {/:Italic https://irsa.ipac.caltech.edu/docs/program\_interface/ztf\_lightcurve\_api.html}'
set label 4 at graph 0.04, 0.70 tc lt 3 font "Times-Roman,12"
